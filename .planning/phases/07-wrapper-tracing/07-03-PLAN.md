...
must_haves:
  truths:
    - "Pass 1 finds `function apiFetch(path) { fetch(...) }` and marks apiFetch as a fetch wrapper"
    - "Pass 2 detects `apiFetch('/api/v1/teams')` and extracts `/api/v1/teams` as the connection path"
    - "Template literals normalize `${orgId}` to `{param}` in extracted paths"
    - "Wrapper map chains work: useData → apiFetch → fetch results in useData as REST wrapper at depth 2"
    - "Wrapper map is returned from build_wrapper_map() and reused across detect_wrapper_calls() calls (cached per-scan)"
    - "extraction_method is wrapper_trace:apiFetch→fetch"
    - "200-line function is included, 201-line function is skipped"
    - "Wrapper depth cap: a level-6 chain is not added to the map"
  artifacts:
    - path: "tests/wrapper_tracing.rs"
      provides: "7 integration tests covering all WRAP-01 through WRAP-07 requirements"
  key_links:
    - from: "tests/wrapper_tracing.rs"
      to: "src/wrapper/mod.rs"
      via: "arcanon_scanner::wrapper::{build_wrapper_map, detect_wrapper_calls, normalize_template_literal, WrapperMap}"
      pattern: "use arcanon_scanner::wrapper"
---

<objective>
Write integration tests that verify all 7 WRAP requirements end-to-end using synthetic fixture files.

Purpose: These tests prove the complete two-pass algorithm works correctly and forms the regression safety net for all future changes to wrapper tracing. One test per requirement, all using concrete code fixtures.

Output: tests/wrapper_tracing.rs with 7 passing tests.
</objective>
...
@.planning/phases/07-wrapper-tracing/07-02-SUMMARY.md

@src/wrapper/mod.rs
@src/patterns/mod.rs

<interfaces>
<!-- Public API available for tests -->
...
From src/patterns/mod.rs (for constructing test registries):
```rust
...
Helper for tests — create FileContext:
```rust
...
<feature>
  <name>Wrapper Tracing Integration Tests</name>
  <files>tests/wrapper_tracing.rs</files>
  <behavior>
    WRAP-01: Pass 1 finds function wrappers around known connection functions
    WRAP-02: Pass 2 detects calls to discovered wrappers with path extraction
    WRAP-03: Library wrapper detection (source scanned as lib_files)
    WRAP-04: Template literal normalization to {param}
    WRAP-05: Wrapper chain depth — multi-level wrappers discovered
    WRAP-06: Wrapper map returned from build_wrapper_map reused across detect calls (per-scan cache)
    WRAP-07: extraction_method "wrapper_trace:{wrapper}→{terminal}" in ConnectionInfo
  </behavior>
  <implementation>
Create `/Users/ravichillerega/sources/arcanon-scanner/tests/wrapper_tracing.rs` with the following 7 tests. Each test uses synthetic FileContext fixtures — no filesystem access needed.

```rust
//! Integration tests for wrapper tracing (Phase 7).
//! One test per WRAP requirement. All tests use synthetic in-memory fixtures.

use arcanon_scanner::patterns::{Detection, Pattern, PatternConfidence, PatternRegistry, TargetExtraction};
use arcanon_scanner::plugin::FileContext;
use arcanon_scanner::types::ExtractionResult;
use arcanon_scanner::wrapper::{build_wrapper_map, detect_wrapper_calls, normalize_template_literal};
...
"#,
    ...
    assert_eq!(info.depth, 1, "apiFetch wraps fetch directly — depth should be 1");
}
...
"#,
    ...
    assert!(map.contains("apiFetch"), "wrapper map must contain apiFetch");

    let result = detect_wrapper_calls(&[caller_file], &map, &HashMap::new());

    assert!(
        !result.connections.is_empty(),
        "Pass 2 should detect the apiFetch call"
    );
...
fn test_wrap_04_template_literal_normalization() {
    ...
    let caller_file = make_file(
        "src/hooks/useOrg.ts",
        r#"import { apiFetch } from '../lib/api';
export async function loadTeams(orgId: string) {
    return apiFetch(`/api/v1/orgs/${orgId}/teams`);
}
"#,
    );
    let all_files = vec![api_file, caller_file.clone()];
...
fn test_wrap_05_wrapper_chain_multi_level() {
    ...
    assert!(map.contains("apiFetch"), "apiFetch must be in map (level 1)");
...
fn test_wrap_06_wrapper_map_reused_across_detect_calls() {
    ...
    assert_eq!(result_a.connections.len(), 1, "File A should detect one wrapper call");
    assert_eq!(result_b.connections.len(), 1, "File B should detect one wrapper call");
    assert_eq!(
        result_a.connections[0].path,
        Some("/api/a".to_string())
    );
    assert_eq!(
        result_b.connections[0].path,
        Some("/api/b".to_string())
    );
}
...
cargo test --test wrapper_tracing -- --nocapture 2>&1 | tail -40
cargo test 2>&1 | grep -E "FAILED|test result"
```
</verification>

<success_criteria>
- All 7 tests in tests/wrapper_tracing.rs pass
- `cargo test --test wrapper_tracing` exits 0
- No regressions in existing tests (`cargo test` overall passes)
- Each test name references its WRAP requirement (test_wrap_01_ through test_wrap_07_)
</success_criteria>

<output>
After completion, create `.planning/phases/07-wrapper-tracing/07-03-SUMMARY.md`
</output>
