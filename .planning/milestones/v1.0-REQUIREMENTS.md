# Requirements Archive: v1.0 Arcanon Scanner v1.0
...
- [x] **VARS-01**: Scanner builds VariableStore from .env files (merge order: .env < .env.local < .env.development < .env.production)
- [x] **VARS-02**: Scanner reads docker-compose environment entries into VariableStore
- [x] **VARS-03**: Scanner reads Kubernetes ConfigMap data into VariableStore
- [x] **VARS-04**: Language plugins can resolve variable names through the store and extract service targets from URL values
- [x] **VARS-05**: Scanner traces variable references through: inline literal, local constant, module-level constant (one import level), .env, compose env, k8s ConfigMap, env var name heuristic
...
- [x] **LPLU-01**: TypeScript plugin detects Express, NestJS, Next.js, Fastify routes and fetch/axios/got client calls via tree-sitter AST
- [x] **LPLU-02**: Python plugin detects FastAPI, Django, Flask routes and httpx/requests/aiohttp client calls via tree-sitter AST
- [x] **LPLU-03**: Go plugin detects net/http, Gin, Echo, Fiber routes and http.Get/grpc.Dial client calls via tree-sitter AST
- [x] **LPLU-04**: Java plugin detects Spring Boot annotations (@RestController, @RequestMapping) and RestTemplate/WebClient calls via tree-sitter AST
- [x] **LPLU-05**: C# plugin detects ASP.NET Core attributes ([ApiController], [HttpGet]) and HttpClient calls via tree-sitter AST
- [x] **LPLU-06**: Rust plugin detects Actix-web, Axum, Rocket routes and reqwest/tonic client calls via tree-sitter AST
- [x] **LPLU-07**: Ruby plugin detects Rails routes.rb, Sinatra routes, and Faraday/Net::HTTP client calls via tree-sitter AST
...
- [x] **LPLU-11**: Language plugins detect industrial protocol calls (Modbus, OPC UA, BACnet, CAN bus, HL7/FHIR) from known library patterns
...
- [x] **DETQ-05**: Plugins that use two-phase extraction (NestJS @Controller prefix, Spring @RequestMapping, ASP.NET [Route]) produce correct full paths
...
- [x] **WRAP-01**: Pass 1 scans function definitions in user code and identifies wrappers around known connection functions (fetch, axios, httpx, etc.)
- [x] **WRAP-02**: Pass 2 detects calls to discovered wrappers and extracts path/URL from arguments including template literals
- [x] **WRAP-03**: Library wrapper detection — scans installed library source to find methods that wrap known connection functions (e.g., JournalClient.append → httpx.post)
- [x] **WRAP-04**: Template literal path extraction normalizes interpolated segments to {param} (e.g., `/api/v1/orgs/${orgId}/teams` → `/api/v1/orgs/{param}/teams`)
- [x] **WRAP-05**: Wrapper map chains — if function A wraps function B which wraps fetch, A is marked as a REST wrapper (multi-level)
- [x] **WRAP-06**: Wrapper map is cached per-scan and shared across all files in the language
- [x] **WRAP-07**: Wrapper-traced connections include extraction_method "wrapper_trace:{wrapper}→{terminal}" and the extracted path in ConnectionInfo
...
| PTRN-01 | Phase 5 | Complete |
| PTRN-02 | Phase 5 | Complete |
| PTRN-03 | Phase 5 | Complete |
| PTRN-04 | Phase 5 | Complete |
| PTRN-05 | Phase 5 | Complete |
| PTRN-06 | Phase 5 | Complete |
| PTRN-07 | Phase 5 | Complete |
| LRES-01 | Phase 6 | Complete |
| LRES-02 | Phase 6 | Complete |
| LRES-03 | Phase 6 | Complete |
| LRES-04 | Phase 6 | Complete |
| LRES-05 | Phase 6 | Complete |
| LRES-06 | Phase 6 | Complete |
| WRAP-01 | Phase 7 | Complete |
| WRAP-02 | Phase 7 | Complete |
| WRAP-03 | Phase 7 | Complete |
| WRAP-04 | Phase 7 | Complete |
| WRAP-05 | Phase 7 | Complete |
| WRAP-06 | Phase 7 | Complete |
| WRAP-07 | Phase 7 | Complete |
...
*Last updated: 2026-04-05 — added PTRN/LRES/WRAP traceability, fixed stale checkboxes*
